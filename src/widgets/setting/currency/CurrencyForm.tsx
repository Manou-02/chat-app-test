...
export const CurrencyForm: FC<PropsType> = ({ handleSuccess, selected }) => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (!initialState?.id) {
        const { data, status } = await CurrencyServices.createCurrencies(
          values
        );
        ...
        const { data, status } = await CurrencyServices.updateCurrencies(
          initialState
        );
