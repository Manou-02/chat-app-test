...
import { CentersService } from "@/features/setting/centers/services/centers.services";
...
export const CentersForm: FC<PropsType> = ({
  handleSuccess,
  selected,
}: PropsType) => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      handleLoading();
      if (!selected) {
        const { data, status } = await CentersService.createCenters(values);
        ...
        const { data, status } = await CentersService.updateCenters(
          initialState
        );
