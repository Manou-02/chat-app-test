...
export const CustomerTypeForm: FC<PropsType> = ({
  handleSuccess,
  selected,
}) => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (!initialState?.id) {
        const { data, status } = await CustomerTypeServices.createCustomerType(
          values
        );
        ...
        const { data, status } = await CustomerTypeServices.updateCustomerType(
          initialState
        );
