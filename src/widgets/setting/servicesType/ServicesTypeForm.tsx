...
import { ServiceTypeService } from "@/features/setting/serviceType/services/ServiceType.services";
...
export const ServicesTypeForm: FC<PropsType> = ({
  handleSuccess,
  selected,
}) => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (!initialState?.id) {
        const { data, status } = await ServiceTypeService.createServiceType(
          values
        );
        ...
        const { data, status } = await ServiceTypeService.updateServiceType(
          initialState
        );
